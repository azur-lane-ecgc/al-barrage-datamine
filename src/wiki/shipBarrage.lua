...
function p.shipBarrage(frame)
    ...
    local barrage_type = args and args.barrage_type
    ...
    if locCol then
        ...
        cols = {
            "Skill", label, "Type", "Stat", "Ammo", "DMG", "Count",
            "Coef", "Light", "Medium", "Heavy", "Aim", "Notes", "Effect"
        }
    else
        cols = {
            "Skill", "Type", "Stat", "Ammo", "DMG", "Count",
            "Coef", "Light", "Medium", "Heavy", "Aim", "Notes", "Effect"
        }
    end
        ...
        if entry then
            ...
            for _, barrage in ipairs(entry.barrages or {}) do
                ...
                for i, part in ipairs(parts) do
                    -- Skip if barrage_type filter is set and doesn't match
                    if barrage_type and part.type ~= barrage_type then
                        goto continue
                    end
                    ...
                    -- Type
                    row:tag("td"):wikitext(string.upper(part.type or "")):done()

                    -- Stat (all types have this)
                    row:tag("td"):wikitext(string.upper(part.stat or "")):done()

                    -- Weapon/Slash specific fields (Damaging Barrage)
                    if part.type == "weapon" or part.type == "slash" then
                        -- Ammo + modifiers
                        ...
                        -- Summon type - leave damaging fields empty
                        row:tag("td"):wikitext(""):done() -- Ammo
                        row:tag("td"):wikitext(""):done() -- DMG
                        row:tag("td"):wikitext(""):done() -- Count
                        row:tag("td"):wikitext(""):done() -- Coef
                        row:tag("td"):wikitext(""):done() -- Light
                        row:tag("td"):wikitext(""):done() -- Medium
                        row:tag("td"):wikitext(""):done() -- Heavy
                    end
                    ...
                    -- Add summon-specific stats for summon type
                    if part.type == "summon" then
                        local summonStats = {}
                        if part.hp then table.insert(summonStats, "HP: " .. tostring(part.hp)) end
                        if part.armor then table.insert(summonStats, "Armor: " .. tostring(part.armor)) end
                        if part.hull then table.insert(summonStats, "Hull: " .. tostring(part.hull)) end
                        if part.fp then table.insert(summonStats, "FP: " .. tostring(part.fp)) end
                        if part.trp then table.insert(summonStats, "TRP: " .. tostring(part.trp)) end
                        if part.aa then table.insert(summonStats, "AA: " .. tostring(part.aa)) end
                        if part.avi then table.insert(summonStats, "AVI: " .. tostring(part.avi)) end
                        if part.rld then table.insert(summonStats, "RLD: " .. tostring(part.rld)) end
                        if part.hit then table.insert(summonStats, "HIT: " .. tostring(part.hit)) end
                        if part.eva then table.insert(summonStats, "EVA: " .. tostring(part.eva)) end
                        if #summonStats > 0 then
                            table.insert(notesList, table.concat(summonStats, ", "))
                        end
                    end

                    -- Add weapon/slash specific notes
                    ...
                    if part.fix_damage and part.fix_damage > 0 then table.insert(notesList, "Fixed DMG: " .. tostring(part.fix_damage)) end

                    -- Add slash-specific notes
                    if part.type == "slash" then
                        if part.level_of then table.insert(notesList, "Level: " .. part.level_of) end
                        if part.movement_type == 1 then table.insert(notesList, "Stationary") end
                        if part.movement_type == 0 then table.insert(notesList, "Traveling") end
                        if part.clears then table.insert(notesList, "Clears: " .. tostring(part.clears)) end
                        if part.fix_damage and part.fix_damage > 0 then table.insert(notesList, "Fixed DMG: " .. tostring(part.fix_damage)) end
                        if part.life_time then table.insert(notesList, "Duration: " .. tostring(part.life_time) .. "s") end
                        if part.range then table.insert(notesList, "Range: " .. tostring(part.range)) end
                    end
...
function p.list(frame)
    ...
    local barrage_type = args.barrage_type
    local locCol = tonumber(args.locCol) == 1
