        ...
        for sid_str, entry in data.items():
            ...
            for vi, variant in enumerate(entry["barrages"]):
                ...
                for pi, part in enumerate(variant["parts"]):
                    ...
                    matching_part = None
                    for sp in sv["parts"]:
                        if sp.get("damage") == part.get("damage") and sp.get(
                            "count"
                        ) == part.get("count"):
                            matching_part = sp
                            break
                    ...
                    part["aim_type"] = (
                        matching_part.get("targetting", 0) if matching_part else 0
                    )
...
def lua_convert(input_path: str, output_path: str) -> None:
    ...
    def to_lua(v: Any, indent: str = "") -> str:
        if isinstance(v, list):
            ...
            for k in sorted(
                v.keys(), key=lambda x: (x.isdigit(), int(x) if x.isdigit() else x)
            ):
                val = v[k]
